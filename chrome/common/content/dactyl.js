...
var Dactyl = Module("dactyl", XPCOM(Ci.nsISupportsWeakReference, ModuleBase), {
    ...
    cleanup: function () {
        ...
        styles.system.add("cleanup-sheet", config.styleableChrome, /* Preprocessors FTW. */ '\n\
    ...
    urlish: Class.Memoize(() => util.regexp(/* Preprocessors FTW. */ '\n\
            ^ (\n\
                <domain>+ (:\\d+)? (/ .*) |\n\
                <domain>+ (:\\d+) |\n\
                <domain>+ \\. [a-z0-9]+ |\n\
        ...
        ', "ix", {
        domain: util.regexp(String.replace(/* Preprocessors FTW. */ '\n\
            [^\n\
